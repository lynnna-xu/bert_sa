# coding=utf-8
# /data/miniconda3/envs/tf1.11.0/bin/python
...
TRAINED_CLASSIFIER_DIR="../output/sa_output/model.ckpt-617"
...
flags.DEFINE_string("task_name", "sa", "The name of the task to train.")

flags.DEFINE_string("vocab_file", BERT_BASE_DIR+"vocab.txt",
                    "The vocabulary file that the BERT model was trained on.")

flags.DEFINE_string("init_checkpoint", TRAINED_CLASSIFIER_DIR, 
...
class SAProcessor(DataProcessor):
  """Processor for sentiment analysis (MEDIAV version)."""
  import pandas as pd
  def get_train_examples(self, data_dir):
    """See base class."""
  ...
  def get_dev_examples(self, data_dir):
    """See base class."""
    return self._create_examples(
        pd.read_csv(os.path.join(data_dir, "dev.csv")), "dev")

  def get_test_examples(self, data_dir):
    """See base class."""
    return self._create_examples(
      pd.read_csv(os.path.join(data_dir, "dev1204.csv")), "test")
  ...
  def _create_examples(self, df, set_type):
    ...
    for i in range(df.shape[0]): 
      guid = "%s-%s" % (set_type, i)
      if set_type == "test":
        text_a = tokenization.convert_to_unicode(str(df['content'][i]))
        label = "0"
      else:
        text_a = tokenization.convert_to_unicode(str(df['content'][i]))
        label = tokenization.convert_to_unicode(str(df['label'][i]))
      examples.append(
...
def predict_online(line):
    ...
    """
    text_a = line
    example= InputExample(guid=0, text_a=text_a, label="0")
